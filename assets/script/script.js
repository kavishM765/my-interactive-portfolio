...
function openEmailModal() { 
    // UPDATED: Now also gets and sets the location
    const loggedInEmail = document.getElementById('email').value;
    const loggedInLocation = document.getElementById('location').value;
    ...
    if(visitorLocationField) visitorLocationField.value = loggedInLocation;
...
function initializeApp() {
    ...
    detailPages.forEach(key => {
        // Fallback for safety, though should not be needed with the HTML fix
        const pageKey = pageContent[key] ? key : 'journey'; 
        const detailContent = `<div class="page-content-wrapper"><div class="mx-auto max-w-7xl">${pageContent[pageKey]}</div></div>`;
